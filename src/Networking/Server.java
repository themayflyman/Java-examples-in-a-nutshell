...
public class Server {
    ...
    Map services;                   // Hashtable mapping ports to Listener
    ...
    public class Listener extends Thread {
            ...
            throws IOException
        {
            ...
            // give it a non-zero timeout so accept() ca be interrupted
    ...
    protected synchronized void addConnection(Socket s, Service service) {
        ...
        if (connections.size() >= maxConnections) {
            ...
            log("Connected to " + s.getInetAddress().getHostName() +
    ...
    public class Connection extends Thread {
         ...
         * constructor to create a thread to handle connection. Connection
     ...
     * will be passed an input stream and an output stream to the client. ITs
     ...
     * running on more than one port, there will be typically be different
    ...
    /**
     * A very simple service. It displays the currect time on the server
     ...
     * impleneted earlier in this chapter. It echos back the client's
     * HTTP request.
     */
    ...
    /** TODO
     * This service responds to GET requests like those generated by the program
     * in Exercise 5-2 HttpClient2.
     * It should use the HTTP 1.0 protocol or a later version. This means that it
     * expects GET requests to be followed by header lines and terminated by a
     * blank line. And it means that the responses to GET requests should begin
     * with a version number and status code. This status line is followed by
     * header lines, which are terminated with a blank line.
     * THe service should be started with a directory specified on the command
     * line and should serve files relative to this directory. When a client
     * requests a file in or beneath the directory, the server should return
     * the contents of the file, but should first output Content-Type, Content-
     * Length, and Last-Modified header lines. For this exercise, assume that
     * files with an extension of .html have a content type of text/html, and
     * that all other files are text/plain. If a client requests a file that
     * doesn't exist, your server should return an appropriate error code and
     * message.
     */
    public static class Exercise5_5 implements Service {
        public void serve(InputStream i, OutputStream o) throws IOException {
            BufferedReader in = new BufferedReader(new InputStreamReader(i));
            PrintWriter out = new PrintWriter(o);
            out.print("HTTP/1.0 200 \n");
     ...
     * in capital letter. Only one client is allowed to connect to this
     * service at a time.
     */
    public static class Control implements Service {
        ...
        /**
         * This is the sever method that provides the service. It reads a
         * line from the client, and uses java.util.StringTokenizer to parse it
