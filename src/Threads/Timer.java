...
public class Timer {
    ...
    public Timer(boolean isDaemone) {
        timer = new TimerThread(isDaemone);    // TimerThread is defined below
    ...
    /* Schedule a periodic execution starting at the specified delay */
    ...
    class TimerThread extends Thread {
        ...
        public void run() {
            ...
            while(!stopped) {
                ...
                if (readyToRun != null) {
                    if (readyToRun.cancelled) {   // If it was canceled, skip
                        readyToRun = null;
                        continue;
                    }
                    // Run the task
                    ...
                    // again, then insert it back into the set of the tasks.
    ...
    public static class Test {
        public static void main(String[] args) {
            final TimerTask t1 = new TimerTask() {
                @Override
                public void run() {  // Task 1: print "boom"
                    System.out.println("boom");
                }
            };
            final TimerTask t2 = new TimerTask() {
                @Override
                public void run() {  // Task 2: print "BOOM"
                    System.out.println("\tBOOM");
                }
            };
            final TimerTask t3 = new TimerTask() {
                @Override
                public void run() {  // Task 3: cancel the tasks
                    t1.cancel(); t2.cancel();
                }
            };
            ...
            // down from 5, then destory the timer, which, since it is
